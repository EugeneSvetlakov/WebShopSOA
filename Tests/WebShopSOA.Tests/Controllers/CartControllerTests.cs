...
namespace WebShopSOA.Tests.Controllers
{
    ...
    {
        [TestMethod]
        public void CheckOut_ModelState_Invalid_Return_ViewModel()
        {
            ...
            var model = Assert.IsAssignableFrom<OrderDetailsViewModel>(view_result.ViewData.Model);

            Assert.Equal(expected_model_name, model.OrderViewModel.Name);
        }

        [TestMethod]
        public void CheckOut_Cals_Services_And_Return_Redirect()
        {
            var user =
                new ClaimsPrincipal(
                    new ClaimsIdentity(
            ...
            Assert.Equal(nameof(CartController.OrderConfimed), redirect_result.ActionName);
